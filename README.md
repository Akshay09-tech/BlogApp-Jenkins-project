...
🖥️ App Preview
![image alt](https://github.com/Akshay09-tech/BlogApp-Jenkins-project/blob/bce7fa26bcac20e6b12a516280543f295798d77b/Screenshot%202026-03-21%20162402.png)
...
🔧 Tech Stack
LayerTechnologyFrontendHTML, CSS, JavaScript, NginxBackendPython, Flask, Flask-CORSDatabaseSQLiteContainerizationDockerImage RegistryDockerHubCI/CDJenkinsOrchestrationKubernetes (Minikube)CloudAWS EC2 (Ubuntu 22.04)Version ControlGitHub + Webhooks
...
Why this is needed:
Minikube is started by the ec2-user, so all the Kubernetes certificates and config files are stored in the ec2-user home directory. Jenkins runs as a separate jenkins user which has its own home directory and cannot access ec2-user files by default.
What needs to be done:

The entire .minikube folder (which contains all TLS certificates) must be copied into the Jenkins home directory at /var/lib/jenkins/.minikube
The .kube/config file (which tells kubectl where the cluster is and which certificates to use) must be copied to /var/lib/jenkins/.kube/config
The config file contains hardcoded paths pointing to ec2-user home — these paths must be updated to point to the new Jenkins home location
Ownership of both folders must be given to the jenkins user so it can read them
The original .minikube folder permissions must be loosened so Jenkins can access the certificates

Once this is done, Jenkins can successfully run kubectl commands inside the pipeline.
...
How CI/CD pipeline works end to end in real world
Docker image build, tag and push to registry
Kubernetes Deployment, Service and Ingress concepts
Jenkins pipeline stages and credentials management
Linux file permissions and user groups
Debugging real production-like errors independently
